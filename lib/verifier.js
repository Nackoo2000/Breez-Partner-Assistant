...
export function evaluateTriggers({ classification, partnerName }) {
  ...
  if (isSuggestedOOO) {
    const options = buildOptionsForOOO({ suggestedPerson, alternativePerson });
    if (options.length >= 2) {
      return { escalate: true, reason: 'ooo', options };
    }
  }
...
function buildOptionsForOOO({ suggestedPerson, alternativePerson }) {
  // Primary is OOO — don't offer them. Offer alternative if we have one, plus
  // a thread-reply escape hatch. Option list needs at least two entries or
  // there's nothing to vote on.
  const opts = [];
  if (alternativePerson && alternativePerson !== suggestedPerson && TEAM_ID_BY_NAME[alternativePerson]) {
    opts.push({ emoji: 'one', person: alternativePerson, rationale: 'close alternative (primary is OOO)' });
  }
  opts.push({ emoji: opts.length ? 'two' : 'one', person: null, rationale: 'someone else (thread reply with name)' });
  // If we only have the escape hatch, signal no_escalation — best-guess card still posts with OOO banner from the classifier
  return opts;
}
...
export function buildVerifierBlocks({ reason, options }) {
  ...
  const reasonLine = {
    low_conf:        `:mag: Not confident on this routing — ${royMention}, can you confirm?`,
    close_alt:       `:mag: Two close candidates — ${royMention}, which one?`,
    ooo:             `:palm_tree: Suggested owner is OOO — ${royMention}, who should cover?`,
    unknown_partner: `:wave: New partner (no routing history) — ${royMention}, who owns this?`,
  }[reason] || `:mag: Needs verification — ${royMention}?`;

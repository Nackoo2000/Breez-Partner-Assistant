...
async function handleMessage(message, slackClient) {
  ...
  // Register supergroups on any update — including service messages — so the
  // group is known before the first real text message arrives. No API call.
  if (chatId.startsWith('-100')) {
    try { touchTelegramGroup(chatId, chatTitle); } catch {}
  }

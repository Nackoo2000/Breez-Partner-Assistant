...
async function runPatternAnalysis(slackClient) {
  ...
  const digest = resolved.map(r => {
    ...
    const verifierTag = (r.correction_reason && /^verifier:/.test(r.correction_reason))
      ? (r.corrected_by === 'Roy' && r.corrected_person === 'Roy'
          ? ' [VERIFIED BY ROY — authoritative]'   // Roy confirming his own routing
          : ' [team-verified]')                     // everyone else, or Roy confirming others
      : '';
...
Weight the evidence:
- Rows tagged [VERIFIED BY ROY — authoritative] are high-trust: Roy confirmed his own ownership of this case. One such row counts as ~3 regular corrections.
